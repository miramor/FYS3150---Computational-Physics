...
The backward substitution starts at the last row of the matrix. Here, the expression for $u_{n-1}$ in the last row becomes immediately clear
\begin{equation*}
\begin{aligned}
...
\begin{aligned}
\tilde{g}_i &= g_i - a_{j-1} \cdot \tilde{g}_{i-1} / \tilde{b}_{i-1}  \hspace{0.5cm} \mathrm{for} \hspace{0.1cm} i=2,..., n-1 \\
\tilde{g}_i &= g_i - a_{j-1} \cdot \tilde{g}_{i-1} / i/(i-1) \\
\tilde{g}_i &= g_i - (i-1) \cdot a_{j-1} \cdot \tilde{g}_{i-1} / i
\end{aligned}
\end{equation*}
Similarly, the backward loop is now given as......
\begin{table}
    \begin{tabular}{|l|l|l|l|}
        \hline
        FLOPs for         & Forward   & Backward  & Total    \\ \hline
        $\mathbf{b}$      & $3 (n-2)$ & 0         & -        \\ \hline
        $\mathbf{g}$      & $3 (n-2)$ & 0         & -        \\ \hline
        $\mathbf{u}$      & 0         & $3 (n-2)$ & -        \\ \hline
        Total for general & -         & -         & 27 (n-2) \\
        \hline
    \end{tabular}
\end{table}
...
\item Thomas algorith with backward substitution (general + special case): for (i=-2,n) compute $\mathbf{u}$
