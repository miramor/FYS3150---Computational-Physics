...
vec Solver::TotalAccelerationOnPlanet(vec<Planet> planets, const Planet& planet, int index){ //calculate total acceleration on planet
  ...
  for(int i =0; i < planets.size(); i++){ // find neighbour planets and calculate acceleration 
    if(planets[i] != planet){
      accel += planet.gravitationForce(planets[i], index) //fetch gravitationalForce on planet due to neighbour planet for a given time (=index)
    } else{
...
void Solver::EulerCromer(int N, double t_end, vec<Planet> planets){
  double h = t_end/N; //stepsize
    for (int j = 0; j < N; j++){
        for(k=0; k < planets.size(); k++){ //for every planet compute position and velocity at specific time j
          vec h_accel = h*TotalAccelerationOnPlanet(planets, planets[k], j) //fetch planet's acceleration vector [a_x, a_y, a_z] times h at a given time j
...
void Solver::VelocityVerlet(int N, double t_end, vec<Planet> planets){
  double h = t_end/N; //stepsize
  double h_2 = h/2.0; //stepsize
    for (int j = 0; j < N; j++){
        for(k=0; k < planets.size(); k++){ //for every planet compute position and velocity at specific time j
          vec accel = TotalAccelerationOnPlanet(planets, planets[k], j) //fetch planet's acceleration vector [a_x, a_y, a_z] times h at a given time j
          ...
          vec accel_next = TotalAccelerationOnPlanet(planets, planets[k], j+1) //fetch planet's acceleration vector [a_x, a_y, a_z] times h at a given time j+1
