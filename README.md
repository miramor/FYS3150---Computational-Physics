...
Using *Option 2* a folder named **"Results"** is added for the programs which takes hours to run. In our case, the observable files were computed using 15 million cycles. Currently *plot.py* uses these files. If plots for stabilization development are needed run *Option 1* with e.g 400 cycles. Then run *plot_stabi()* function at line 247 in *plot.py*. Similarly, a histogram for the probability distribution can be plotted by uncommenting the *plot_hist_nump()* function. We now used 100 000 cycles instead. (You may need to change T in title, annotation location depends on which temp used and savefig name.)
