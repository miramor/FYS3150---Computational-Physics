...
void SIRS::specMC_VD_VAC(int MC_cyc, double e_, double d_, double dI, double f_){ //MC with e and d
  MC_cycles = MC_cyc;
  d = d_;
  e = e_;
  d_I = dI;
...
vec SIRS::derivatives(vec yt){ //Basic example
  // S, I, R = y(0), y(1), y(2)
...
vec SIRS::derivatives2(vec yt){ //Vital dynamics
...
vec SIRS::derivatives3(vec yt){ //Vaccination
  // S, I, R = y(0), y(1), y(2)
...
vec SIRS::derivatives4(vec yt){ //both VD and Vac
  // S, I, R = y(0), y(1), y(2)
  dy(0) = c*yt(2) - yt(0)*(a*yt(1)/N+d)+e*N - f;
  dy(1) = yt(1)*(a*yt(0)/N - b-d-d_I);
  dy(2) = b*yt(1) - (c+d)*yt(2) + f;
  return dy;
}
...
//
  ...
  //cout << "Last death: " << deadDis(num_pts-1) << endl;
  //cout << deadDis << endl;
  //cout << S_mc << endl;
  deadDis_final = deadDis(num_pts-1);
  //deadDis_final = I_mc(num_pts-1);

  S_final = S_mc(num_pts-1);
  I_final  = I_mc(num_pts-1);
  R_final = R_mc(num_pts-1);
}
...
void SIRS::rk4(bool useVD){
  ...
  else if(useV == true && useVD == true){
    K1 = dt*derivatives4(y);
    K2 = dt*derivatives4(y+0.5*K1);
    K3 = dt*derivatives4(y+0.5*K2);
    K4 = dt*derivatives4(y+K3);
    y = y + (K1 + 2.0*K2 + 2.0*K3 + K4)/6.0;
  }
}
